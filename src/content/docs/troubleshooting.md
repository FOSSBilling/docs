...
When troubleshooting, you may need to reference logs generated by FOSSBilling. The location of these files can be found on our [FAQ](/docs/faq#where-can-i-find-error-logs).
...
- Verify you loaded the NGINX [configuration file](/docs/getting-started/installation#webserver-configurations).
...
1. Review the [changelog](/docs/changelog) for any breaking changes or for any manual actions required.
...
1. Check for any errors in the FOSSBilling [error log](/docs/faq#where-can-i-find-error-logs).
...
3.  Remove any PHP versions less than 8.2 to ensure it can't use an incorrect version.
...
- Verify you are using the administrator login page as the client & admin areas are separate. The default admin prefix is `/admin` (https://example.com/admin).
- [Manually reset your admin password](/docs/maintaining-fossbilling/admin-manual-reset).
